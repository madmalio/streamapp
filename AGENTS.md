...
- **Current Pluto Stability Baseline**: Keep Pluto on direct in-app playback but resolve through backend `GET /api/proxy/m3u8?url=...&best=1` so playlists are normalized (absolute variant URLs, subtitle tracks stripped, highest-bandwidth variant selected) before opening in `media_kit`.
